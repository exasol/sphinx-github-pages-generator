...
def test_pushing_to_existing_docu_branch():
    # different sources?
    pass
...
def test_no_new_push_and_commit_if_no_changes():
    ...
    with TemporaryDirectory() as tempdir2: # neccesary. because worktree points to old temp non existing folder. TOdo fiy worktree cleanup
        os.chdir(tempdir2)
        user_name, user_access_token = setup_test_repo()
        doc_dir = run(["git", "rev-parse", "--show-toplevel"], capture_output=True, text=True, check=True)
        os.chdir(f"{doc_dir.stdout[:-1]}/doc")
        ...
        # the source files where not updated, so no new commit should be pushed to the remote
