...
def checkout_target_branch_as_worktree(target_branch, worktree, push_origin):
    ...
    if target_branch_exists.returncode == 0:
        ...
        gh_pages_main_branch_exists = run(
            ["git", "show-ref", f"refs/heads/{push_origin}/{gh_pages_main_branch}", "||", "echo"], capture_output=True,
            text=True)
...
def git_commit_and_push(worktree, push_origin, push_enabled, source_branch, output_dir, current_commit_id,
                            target_branch):
    currentworkdir = os.getcwd()
    print(currentworkdir)
    os.chdir(worktree)
    ...
    if 1 == changes_exists.returncode:
        ...
        run(["git", "commit", "--no-verify", "-m",
             f"Update documentation from source branch {source_branch} with commit id {current_commit_id}"], check=True)
